...
class ProcessorThread(threading.Thread):
    ...
    def update_database(self, identifier, name, developer, rating, rating_count, update_date, version, category, downloads, size, price, content_rating, permissions):
        ...
        # try update app
        self.db_cursor.execute('UPDATE "public"."applications" SET last_time_processed = now(), name = '+name+', developer = '+developer+', rating = '+rating+', rating_count = '+rating_count+', update_date = '+update_date+', version = '+version+', category = (SELECT id FROM categories WHERE name = '+category+'), downloads = '+downloads+', size = '+size+', price = '+price+', content_rating = '+content_rating+' WHERE identifier = \''+identifier+'\';')
        self.logger.debug('UPDATE "public"."applications" SET last_time_processed = now(), name = '+name+', developer = '+developer+', rating = '+rating+', rating_count = '+rating_count+', update_date = '+update_date+', version = '+version+', category = (SELECT id FROM categories WHERE name = '+category+'), downloads = '+downloads+', size = '+size+', price = '+price+', content_rating = '+content_rating+' WHERE identifier = \''+identifier+'\';')
        
        # try insert app
        self.db_cursor.execute('INSERT INTO "public"."applications" (identifier, last_time_processed, name, developer, rating, rating_count, update_date, version, category, downloads, size, price, content_rating) SELECT \''+identifier+'\', now(), '+name+', '+developer+', '+rating+', '+rating_count+', '+update_date+', '+version+', (SELECT id FROM categories WHERE name = '+category+'), '+downloads+', '+size+', '+price+', '+content_rating+' WHERE NOT EXISTS (SELECT 1 FROM "public"."applications" WHERE identifier = \''+identifier+'\');')
